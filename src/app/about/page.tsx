...
function Ab({ setPage }: { setPage: (id: string) => void }) {
  ...
  const tgColor = (tg: string) => ({
    bg: tg==="투자"?"rgba(168,149,134,.1)":tg==="수상"?"rgba(16,185,129,.08)":tg==="현재"?"rgba(200,168,130,.12)":"var(--alt)",
    fg: tg==="투자"?"var(--br)":tg==="수상"?"var(--gn)":tg==="현재"?"var(--bw)":"var(--tm)"
  });
              ...
              <div style={{flex:1,padding:"16px 20px 16px 0",display:"flex",flexDirection:"column",justifyContent:"center",gap:items.length > 1 ? 10 : 0}}>
                {items.map((it, ii) => (
                  <div key={it.id} style={{display:"flex",alignItems:"center",gap:10,...(ii > 0 ? {borderTop:"1px solid var(--blt)",paddingTop:10} : {})}}>
                    ...
                    <span style={{fontFamily:"var(--fd)",fontSize:11,padding:"3px 8px",borderRadius:4,background:tgColor(it.tg).bg,color:tgColor(it.tg).fg,fontWeight:600,flexShrink:0,whiteSpace:"nowrap"}}>{it.tg}</span>
                  </div>
                ))}
              </div>
            </div>

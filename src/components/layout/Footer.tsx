  ...
  return <footer style={{ background: "var(--nd)", color: "#fff", padding: "80px 24px 40px" }}>
    <div style={{ maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 48, marginBottom: 64 }}>
        ...
        <div><h4 style={{ fontFamily: "var(--fd)", fontWeight: 600, fontSize: 14, marginBottom: 20, color: "rgba(255,255,255,.35)", textTransform: "uppercase", letterSpacing: ".05em" }}>서비스</h4>{[{nm:"FLOW PAY",id:"flowpay"},{nm:"FLOW SCORE",id:"flowscore"},{nm:"FLOW POINT",id:"flowpoint"}].map(s=><div key={s.nm} onClick={()=>setPage(s.id)} style={{fontSize:14,color:"rgba(255,255,255,.6)",marginBottom:12,cursor:"pointer",transition:"color .2s"}} onMouseEnter={e=>e.currentTarget.style.color="#fff"} onMouseLeave={e=>e.currentTarget.style.color="rgba(255,255,255,.6)"}>{s.nm}</div>)}</div>
        <div><h4 style={{ fontFamily: "var(--fd)", fontWeight: 600, fontSize: 14, marginBottom: 20, color: "rgba(255,255,255,.35)", textTransform: "uppercase", letterSpacing: ".05em" }}>회사</h4>{[["about","회사 소개"],["faq","FAQ"],["esg","ESG"],["notices","공지사항"],["terms","이용약관"],["contact","문의하기"]].map(([id,l])=><div key={id} onClick={()=>setPage(id)} style={{fontSize:14,color:"rgba(255,255,255,.6)",marginBottom:12,cursor:"pointer",transition:"color .2s"}} onMouseEnter={e=>e.currentTarget.style.color="#fff"} onMouseLeave={e=>e.currentTarget.style.color="rgba(255,255,255,.6)"}>{l}</div>)}<div onClick={()=>setPage("privacy")} style={{fontSize:14,color:"rgba(255,255,255,.6)",marginBottom:12,cursor:"pointer",transition:"color .2s"}} onMouseEnter={e=>e.currentTarget.style.color="#fff"} onMouseLeave={e=>e.currentTarget.style.color="rgba(255,255,255,.6)"}>개인정보처리방침</div></div>
